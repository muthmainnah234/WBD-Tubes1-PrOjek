...
$id = $_GET['id_active'];
$initurl = "hide2.php?id_active=" . $id;
...
<div>
  <div class="makeorder">TRANSACTION HISTORY</div>
    <form action="<?php echo $initurl ?>" method="post">
      <div class="tab2">
        <button class="tablinks" onclick="sambung('history.php', '<?php echo $id; ?>')">MY PREVIOUS ORDER</button>
        <button class="tablinks active" onclick="sambung('history2.php', '<?php echo $id; ?>')">DRIVER HISTORY</button>
      </div>
      <div class="container">
          <div class = "box">
              <?php
                  //Fetch products from the database
                  $results = $db->query("SELECT distinct IDTransaksi,IsHide,name, LokasiAwal, LokasiTujuan, transaction.Rating, comment, DatePosted, imagepath FROM ((user join transaction) join userimage) where user.ID = transaction.IDPenumpang and IDDriver = $id and userimage.ID = user.ID");
                  $num = 0;
              ...
              <div class = "clearfix">
                  ...
                  <div class="driverinfo">
                    ...
                    <br>
                    <label class="rating2" id="rating2"> gave you <div style=color:yellow;display:inline-block><?php echo $row['Rating']; ?></div> stars for this order</label>
                    <br>
                    <label class="comment">and left comment : <br>
